...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            for particle in self.particles:
                ...
                counter[particle] = counter[particle] + emissionModel[manDistance]

            if counter.totalCount()!=0:
                self.particles = []
                for numParticles in range(0, self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            particleslist.append(util.sample(newPosDist))
        self.particles=particleslist
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while numOfParticles > 0:
            for chances in permutations:
                self.particles.append(chances)
                numOfParticles-=1
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            weight = 1
            ghost=0
            while ghost<self.numGhosts:
                if noisyDistances[ghost] != None:
                    distance = util.manhattanDistance(particle[ghost], pacmanPosition)
                    weight = weight * emissionModels[ghost][distance]
                else:
                    tmp = list(particle)
                    tmp[ghost] = self.getJailPosition(ghost)
                    particle = tuple(tmp)
                ghost+=1
