...
def enhancedPacmanFeatures(state, action):
    ...
    if action=="Stop":
        features[action]=-0.01
        return features
    for ghostState in curGhostStates:
        ghostPos = ghostState.getPosition()
        if ghostPos == curPos and ghostState.scaredTimer == 0:
            features[action] = -1000
            return features
    foods = state.getFood().asList()
    distance=[]
    for food in foods:
        distance.append(util.manhattanDistance(food, curPos))

    features[action]=1/(min(distance)+0.1)
    return features
